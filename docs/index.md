...
!!! warning "Early development"
    The client, the server, the Laravel bridge, push notifications, Agent Card signing and extensions are done. The server passes the official A2A test kit ([Conformance](reference/conformance.md)), and the SDK talks to the official Python SDK in both directions. A2A 0.3 compatibility is next, and the API may still change before 1.0. Progress is on the [roadmap](project/roadmap.md).
