...
/**
 * Queued runner + database queue + Redis Streams event log, with a real
 * worker. Needs A2A_TEST_REDIS=host:port (skipped otherwise).
 */
final class CrossProcessRedisTest extends CrossProcessTestCase
{
    protected function setUp(): void
    {
        $redis = getenv('A2A_TEST_REDIS');
        if (!is_string($redis) || $redis === '' || !extension_loaded('redis')) {
